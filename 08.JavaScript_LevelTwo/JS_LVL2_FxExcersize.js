...
// Shorter return (!weekday || vacation);
}
...
function caught_speeding(speed, is_birthday){
  //Code Goes Here
  var bdaySpeedReducer = 0;
  if (is_birthday==true) {
    bdaySpeedReducer = -5;
  }
  trueSpeed = speed+bdaySpeedReducer;
  if (trueSpeed<=60) {
    return 0;
  }
  else if (trueSpeed>60 && trueSpeed<=80) {
    return 1;
  }
  else if (trueSpeed>80) {
    return 2;
  }
}
...
function makeBricks(small, big, goal){
  ...
  //shorter = return (small+big*5>=goal)
  //Correct goal%5 >=0 && goal%5-small<=0 && small+big*5>=goal
}
